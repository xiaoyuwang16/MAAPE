...
class MAAPEVisualizer:
    ...
    def print_weight_statistics(self, edge_weights: Dict[Tuple[int, int], float]):
        """Print statistics about edge weights."""
        scaled_weights = np.array(list(edge_weights.values()))
        print("权重分布:")
        print(f"最小值: {np.min(scaled_weights):.4f}")
        print(f"最大值: {np.max(scaled_weights):.4f}")
        print(f"平均值: {np.mean(scaled_weights):.4f}")
        print(f"中位数: {np.median(scaled_weights):.4f}")
        print("\n分位数:")
        for q in [0.1, 0.25, 0.5, 0.75, 0.9]:
            print(f"{q*100}%分位数: {np.quantile(scaled_weights, q):.4f}")
    ...
    def visualize(self, adj_list: Dict[int, Set[int]], sequence_orders: Dict[int, str],
        ...
        # Print statistics
        self.print_weight_statistics(edge_weights_dict)
